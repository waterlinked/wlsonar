The following examples are provided:

- [record_udp_to_file.py](./record_udp_to_file.py)
- [convert_recording.py](./convert_recording.py)
- [trim_recording.py](./trim_recording.py)

The examples require the dev dependencies listed in pyproject.toml. If you have `uv` installed, you can run the examples with for example `uv run python trim_recording.py`.

There is a short test recording included as test data: [ship_short.sonar](/tests/data/ship_short.sonar).